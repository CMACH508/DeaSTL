...
def get_loss(loss):
    if loss == "focalloss":
        return FocalLoss(gamma=0.5, alpha=0.25)
    else:
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if loss_scaler is None:
            ...
            model_params_dict = {id(param): name for name, param in model.named_parameters()}
            for param_group in optimizer.param_groups:
                for param in param_group['params']:
                    param_id = id(param)
                    if param_id in model_params_dict:
                        name = model_params_dict[param_id]
                        device = param.device if param.is_cuda else "CPU"
                        if device== "CPU":
                            print(f"Parameter: {name}, Device: {device}")
                    else:
                        print("Parameter not found in model (may be manually added)")
...
@torch.no_grad()
def evaluate(data_loader, model, device, header='Test:', ch_names=None, metrics=['acc'], is_binary=True, loss_type = None ):
    input_chans = None
    if ch_names is not None:
        input_chans = utils.get_input_chans(ch_names)
    if is_binary:
