...
from engine_for_finetuning import train_one_epoch, evaluate, get_loss
...
def get_args():
    ...
    parser.add_argument('--min_lr', type=float, default=1e-6, metavar='LR',
                        help='lower lr bound for cyclic schedulers that hit 0 (1e-5)') #zl old 1e-6
    ...
    parser.add_argument('--loss_type', default='bceloss')      #zl
    ...
    parser.add_argument('--taskname', default='bect', type=str,
                        help='task: bect | sleep | sleep_bectnor_13_swi')
...
def get_dataset(args):
    ...
    elif args.dataset == 'BECT':
        train_dataset, test_dataset, val_dataset = utils.prepare_BECT_dataset("path/to/BECT/TASK")
        ch_names = ['EEG FP1', 'EEG FP2-REF', 'EEG F3-REF', 'EEG F4-REF', 'EEG C3-REF', 'EEG C4-REF', 'EEG P3-REF', 'EEG P4-REF', 'EEG O1-REF', 'EEG O2-REF', 'EEG F7-REF', \
                    'EEG F8-REF', 'EEG T3-REF', 'EEG T4-REF', 'EEG T5-REF', 'EEG T6-REF', 'EEG A1-REF', 'EEG A2-REF', 'EEG FZ-REF', 'EEG CZ-REF', 'EEG PZ-REF', 'EEG T1-REF', 'EEG T2-REF']
        ch_names = [name.split(' ')[-1].split('-')[0] for name in ch_names]
        args.nb_classes = 1
...
def main(args, ds_init):
    ...
    if args.nb_classes == 1:
        criterion = get_loss(args.loss_type)
    ...
    if args.eval:
        test_stats = evaluate(data_loader_test, model, device, header='Test:', ch_names=ch_names, metrics=metrics, is_binary=(args.nb_classes == 1), loss_type = args.loss_type)
        print(test_stats['accuracy'],test_stats['balanced_accuracy'],test_stats['pr_auc'],test_stats['roc_auc'])
        print(f"======Accuracy: {test_stats['accuracy']}, balanced accuracy: {test_stats['balanced_accuracy']}")
        print(f"======PRAUC: {test_stats['pr_auc']}, AUROC: {test_stats['roc_auc']}")
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            val_stats = evaluate(data_loader_val, model, device, header='Val:', ch_names=ch_names, metrics=metrics, is_binary=args.nb_classes == 1, loss_type = args.loss_type)
