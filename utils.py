...
class BECTLoader(torch.utils.data.Dataset):
...
def prepare_BECT_dataset(root):
    ...
    # prepare training and test data loader
    train_dataset = BECTLoader(os.path.join(root, "train"), train_files)
    test_dataset = BECTLoader(os.path.join(root, "test"), test_files)
    val_dataset = BECTLoader(os.path.join(root, "val"), val_files)
    print(len(train_files), len(val_files), len(test_files))
    return train_dataset, test_dataset, val_dataset
...
STEP_S = 1
FREQ_BANDS = [[0.3,  4],  # delta
              [  4,  8],  # theta
              [  8, 14],  # alpha
              [ 14, 30],  # beta
              [ 30, 70]]   # gamma
DE_PATH = "de_w" + str(STEP_S)


def get_de(raw_data: np.array, sample_freq=200, window_size=1, freq_bands= torch.tensor([[0.3,  4],  # delta
              [  4,  8],  # theta
              [  8, 14],  # alpha
              [ 14, 30],  # beta
              [ 30, 70]]), stft_n=256):
    ...
    def _get_average_psd(energy_graph, freq_bands, sample_freq, stft_n=256):
        start_index = int(torch.floor(freq_bands[0] / sample_freq * stft_n))
        end_index = int(torch.floor(freq_bands[1] / sample_freq * stft_n))
        if(start_index >= 1):
            start_index -= 1
        ave_psd = torch.mean(energy_graph[:,:,:, start_index:end_index] ** 2, axis=-1)
        return ave_psd
    raw_data = raw_data.transpose(1,2).to(device)
    bs, len_time, n_channels, n_samples = raw_data.shape
    point_per_window = int(sample_freq * window_size)
    window_num = int(n_samples // point_per_window)
    psd_feature = torch.zeros((window_num,  bs, len_time, n_channels, len(freq_bands)))
    for window_index in range(window_num):
        start_index, end_index = point_per_window * window_index, point_per_window * (window_index + 1)
        window_data = raw_data[:,:,:, start_index:end_index].to(device)
        hdata = window_data * torch.hann_window(point_per_window, device =device)  #(16, 13, 23, 200)
        fft_data = torch.fft.fft(hdata, n=stft_n)
        energy_graph = torch.abs(fft_data[:, :, :, 0: int(stft_n / 2)])
        for band_index, band in enumerate(freq_bands):
            band_ave_psd = _get_average_psd(energy_graph, band, sample_freq, stft_n) # (16, 13, 23)
            psd_feature[window_index,:,:,:, band_index] = band_ave_psd
    return torch.log2(100 * psd_feature[0]).to(device)




if __name__ == '__main__':
    x = torch.rand(16,23,13,200)
    x = get_de(x)
    print(x.shape)
